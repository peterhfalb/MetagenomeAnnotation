...
# =============================================================================
# submit_dbcan.sh
# Wrapper: submits 04_dbcan.sh as a Slurm array job.
# Can run in parallel with submit_kofam.sh, submit_phibase.sh,
# and submit_diamond_nr.sh after MetaEuk (step 02) is complete.
#
# Usage:
#   bash submit_dbcan.sh \
#     --annotation-dir <path>
#     [--samples "S1,S2,..."]
#     [--test N]
#     [--time HH:MM:SS]
...
mkdir -p "${ANNOTATION_DIR}"/{dbcan,logs/dbcan}
...
if [[ -n "${SAMPLES_FILTER}" ]]; then
    ACTIVE_LIST="${ANNOTATION_DIR}/sample_list_dbcan_run.txt"
    > "${ACTIVE_LIST}"
...
SBATCH_ARGS=(
    --array="${ARRAY_RANGE}"
    --output="${ANNOTATION_DIR}/logs/dbcan/dbcan_%A_%a.out"
    --error="${ANNOTATION_DIR}/logs/dbcan/dbcan_%A_%a.err"
...
JOB_ID=$(sbatch "${SBATCH_ARGS[@]}" "${SCRIPT_DIR}/04_dbcan.sh" | awk '{print $NF}')
echo "Submitted dbCAN3 job: ${JOB_ID}"
echo "Logs: ${ANNOTATION_DIR}/logs/dbcan/dbcan_${JOB_ID}_*.out"
