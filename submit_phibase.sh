...
# =============================================================================
# submit_phibase.sh
# Wrapper: submits 05_phibase.sh as a Slurm array job.
# Can run in parallel with submit_kofam.sh, submit_dbcan.sh,
# and submit_diamond_nr.sh after MetaEuk (step 02) is complete.
#
# Usage:
#   bash submit_phibase.sh \
#     --annotation-dir <path>
#     [--samples "S1,S2,..."]
#     [--test N]
#     [--time HH:MM:SS]
...
mkdir -p "${ANNOTATION_DIR}"/{phibase,logs/phibase}
...
if [[ -n "${SAMPLES_FILTER}" ]]; then
    ACTIVE_LIST="${ANNOTATION_DIR}/sample_list_phibase_run.txt"
    > "${ACTIVE_LIST}"
...
SBATCH_ARGS=(
    --array="${ARRAY_RANGE}"
    --output="${ANNOTATION_DIR}/logs/phibase/phibase_%A_%a.out"
    --error="${ANNOTATION_DIR}/logs/phibase/phibase_%A_%a.err"
...
JOB_ID=$(sbatch "${SBATCH_ARGS[@]}" "${SCRIPT_DIR}/05_phibase.sh" | awk '{print $NF}')
echo "Submitted PHI-base job: ${JOB_ID}"
echo "Logs: ${ANNOTATION_DIR}/logs/phibase/phibase_${JOB_ID}_*.out"
