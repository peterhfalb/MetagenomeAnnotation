...
# =============================================================================
# submit_pfam.sh
# Wrapper: submits 03b_pfam.sh as a Slurm array job.
# Can run in parallel with submit_kofam.sh, submit_dbcan.sh, submit_phibase.sh,
# submit_mmseqs_taxonomy.sh, submit_mycocosm_taxonomy.sh, and
# submit_featurecounts.sh — all depend only on MetaEuk (step 02) completing.
#
# Usage:
#   bash submit_pfam.sh \
#     --annotation-dir <path>
...
mkdir -p "${ANNOTATION_DIR}"/{pfam,logs/pfam}
...
if [[ -n "${SAMPLES_FILTER}" ]]; then
    ACTIVE_LIST="${ANNOTATION_DIR}/sample_list_pfam_run.txt"
    > "${ACTIVE_LIST}"
...
SBATCH_ARGS=(
    --array="${ARRAY_RANGE}"
    --output="${ANNOTATION_DIR}/logs/pfam/pfam_%A_%a.out"
    --error="${ANNOTATION_DIR}/logs/pfam/pfam_%A_%a.err"
...
JOB_ID=$(sbatch "${SBATCH_ARGS[@]}" "${SCRIPT_DIR}/03b_pfam.sh" | awk '{print $NF}')
echo "Submitted Pfam job: ${JOB_ID}"
echo "Logs: ${ANNOTATION_DIR}/logs/pfam/pfam_${JOB_ID}_*.out"
