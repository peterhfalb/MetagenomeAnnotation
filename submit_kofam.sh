...
# =============================================================================
# submit_kofam.sh
# Wrapper: submits 03_kofam.sh as a Slurm array job.
# Can run in parallel with submit_dbcan.sh, submit_phibase.sh,
# and submit_diamond_nr.sh — all depend only on MetaEuk (step 02) completing.
#
# Usage:
#   bash submit_kofam.sh \
#     --annotation-dir <path>
#     [--samples "S1,S2,..."]
#     [--test N]
#     [--time HH:MM:SS]
...
mkdir -p "${ANNOTATION_DIR}"/{kofam,logs/kofam}
...
if [[ -n "${SAMPLES_FILTER}" ]]; then
    ACTIVE_LIST="${ANNOTATION_DIR}/sample_list_kofam_run.txt"
    > "${ACTIVE_LIST}"
...
SBATCH_ARGS=(
    --array="${ARRAY_RANGE}"
    --output="${ANNOTATION_DIR}/logs/kofam/kofam_%A_%a.out"
    --error="${ANNOTATION_DIR}/logs/kofam/kofam_%A_%a.err"
...
JOB_ID=$(sbatch "${SBATCH_ARGS[@]}" "${SCRIPT_DIR}/03_kofam.sh" | awk '{print $NF}')
echo "Submitted KOfamScan job: ${JOB_ID}"
echo "Logs: ${ANNOTATION_DIR}/logs/kofam/kofam_${JOB_ID}_*.out"
